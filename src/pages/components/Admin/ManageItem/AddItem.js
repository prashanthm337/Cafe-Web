...
function AddItem() {
  ...
  const serviceMethod = async (mainURL,method,data,handleSuccess,handleException) => {
    ...
    const handleSave = async (e) => {
      e.preventDefault(); 
      if(!category_name||!item_name||!image||!item_status||!item_price||!item_description){
        alert('please fill out all required fields.');
        return;
      }
      const method = "POST";
          const data = {category_id:category_name,item_name,image,item_status,price:item_price,item_description};
...
return (
  <Container component="main" maxWidth="xm" sx={{backgroundColor: '#FAE6FA',    height: "89%"}}>
    <Box
      sx={{
        display: 'flex',
