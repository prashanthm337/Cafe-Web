...
module.exports = {
        ...
        pool.query(
            ...
            (error, results, fields) => {
                ...
                if (error) {
                    ...
                    err = "EmailId is not registereds";
                    return callBack(err);
                }else if (data.user_password != results[0].user_password) {
                    err = "Password is Incorrect";
                    return callBack(err);
                }
                else {
                    if(queryData[0].user_type == "admin"){
    ...
    forgotpwd: (body, callBack) => {
        pool.query(
            `select user_password,user_email from login_master where user_email = ?`,
            [body.user_email],
            (error, results) => {
                if (error) {
                    return callBack(error);
